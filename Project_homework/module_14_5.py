...
import asyncio
from crud_functions import is_included, add_user, get_all_products, exit_db, conect
import logging
...
async def set_username(message: types.Message, state):
    if is_included(message.text, conection):
...
async def set_age(message: types.Message, state):
    ...
    add_user(conection=conection, **user_data)
    await state.finish()
...
async def get_buying_list(message: types.Message):
    if conection is None:
        ...
        products = get_all_products(conection.cursor())
        for i in products:
